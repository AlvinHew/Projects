...
int main()
{
	...
	model.lambda_ini=1;					//state A
    model.lambda_final=0;					//state B
	...
	for(i=0;i<2;i++){
		...
		for(model.i=0;model.i<model.step;model.i++){
    		...
    		model.sample();                               //initial property 
				...
				if( model.icycl%1000000==0 ) model.sample();                            //determine system property
    ...
    energy=0;
        ...
        for(j=0;j<npart;j++) {
            ...
            if(r2<rc2) {  //within cutoff distance
               r2i= 1/r2;
               r6i= r2i*r2i*r2i;     
               energy +=lambda_1*( (p0*r6i - p1*2)*r6i/12.0 + chg[i]*chg[j]*332.0637*sqrt(r2i));
...
{   //this is almost the same as the energy calculation in force()
    int i,j,k,nbox;
    double r2,r2i,r6i,xr[3],redu,ff,cou;    
    Ep=0;
...
//    P+= Ptail;
    ...
    cout<<null;
    logf<<null;
    sprintf(null,"T %f K, V %f A3, ",T,V/(npart/3));
    cout<<null;
    logf<<null;
    sprintf(null,"Energy (kcal/mol) kinetic %f potential %f total %f",Ek/npart*3,Ep/npart*3,Etot/npart*3);
    cout<<null<<endl;
    logf<<null<<endl;    
...
int MODEL::avg(){
	...
	cout<<endl<<null<<endl;
	logf<<endl<<null<<endl;
	sprintf(null,"T %f K, V %f A3, ",Tavg,Vavg/(npart/3));
	cout<<null;
	logf<<null;
    FEP_result<<null;
	sprintf(null,"E(kcal/mol) Ek %f Ep %f Etot %f ",Ekavg/(npart/3),Epavg/(npart/3),Etotavg/(npart/3));
    cout<<null;
	logf<<null;
    FEP_result<<null;    
