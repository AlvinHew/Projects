...
//    P+= Ptail;
    ...
    cout<<null;
    logf<<null;
    sprintf(null,"Energy (kcal/mol) kinetic %.2f potential %.2f total %.8f",Ek,Ep/npart*3,Etot);
    cout<<null<<endl;
    logf<<null<<endl;    
    ...
    for (i = 0; i < npart; i++) { //position (nm) and velocity (nm/ps)
        if (i%3==0) {
            sprintf(null, "%5d%5s%5s%5d%8.3f%8.3f%8.3f%8.4f%8.4f%8.4f",
                    1, "O", "O", i + 1, x[i][0] / 10, x[i][1] / 10, x[i][2] / 10);
        }
        else {
            sprintf(null, "%5d%5s%5s%5d%8.3f%8.3f%8.3f%8.4f%8.4f%8.4f",
                1, "H", "H", i + 1, x[i][0] / 10, x[i][1] / 10, x[i][2] / 10);
        }
        //sprintf(null, "%5d%5s%5s%5d%8.3f%8.3f%8.3f%8.4f%8.4f%8.4f",
        //    1, "Ar", "Ar", i + 1, x[i][0] / 10, x[i][1] / 10, x[i][2] / 10, v[i][0] / 10, v[i][1] / 10, v[i][2] / 10);
        outf << null << endl;
    }
